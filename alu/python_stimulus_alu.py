...
from alu_uarch import module_alu
    ...
    for op_code, op_name in ops.items():
        for a in boundary_values:
            for b in boundary_values:
                ...
                # Fetch expected results from the Python Golden Model
                ...
                if hw_res != exp_res or hw_z != exp_z or hw_s != exp_s or \
                   hw_ovfl != exp_ovfl or hw_carry != exp_carry:
                    ...
                    dut._log.error(
                        f"  EXPECTED: Res={hex(exp_res)}, Z={exp_z}, S={exp_s}, "
                        f"V={exp_ovfl}, C={exp_carry}"
                    )
                    dut._log.error(
                        f"  HARDWARE: Res={hex(hw_res)}, Z={hw_z}, S={hw_s}, "
                        f"V={hw_ovfl}, C={hw_carry}"
                    )
                    error_count += 1
