...
from decoder_uarch import module_decoder
...
async def automated_decoder_test(dut):
    ...
    for instr_i in test_cases:
        ...
        # 5. Log Output
        dut._log.info(
            f"INPUT: 0x{instr_i:08X} | EXP IMM: 0x{exp_imm_32b:08X} | HW IMM: 0x{hw_imm_32b:08X}"
        )
        ...
        assert hw_imm_32b == exp_imm_32b, (
            f"IMM FAIL! Expected: 0x{exp_imm_32b:08X} | Actual: 0x{hw_imm_32b:08X}"
        )
